...
const GallerySection: React.FC = () => {
    ...
    useEffect(() => {
        const fetchMedia = async () => {
            try {
                ...
                setMedia(highlights.length > 0 ? highlights : response.data.slice(0, 4));
            } catch (error) {
    ...
    const Skeleton = () => (
        <div className="grid grid-cols-2 gap-3 md:gap-6 animate-pulse">
            {[1, 2, 3, 4].map((i) => (
                <div key={i} className="bg-gray-200 rounded-xl md:rounded-2xl h-40 md:h-64" />
            ))}
        </div>
    );
                    ...
                    <div className="grid grid-cols-2 gap-3 md:gap-6">
                        {media.slice(0, 4).map((item, index) => (
                            <Reveal
                                key={item.id}
                                delay={index * 50}
                                className="relative group cursor-pointer overflow-hidden rounded-xl md:rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 bg-gray-100"
                            >
                                    ...
                                    {item.media_type === 'image' ? (
                                        <div className="relative">
                                            ...
                                            <img
                                                ...
                                                className={`w-full h-40 md:h-64 object-cover transform group-hover:scale-110 transition-all duration-500 ${
